...
测试DAOExceptionHandler类的各种异常处理功能, 包括:
1. 不同类型异常的转换和处理
2. 错误消息格式化
3. 日志记录功能
4. 装饰器模式
...
class TestDAOExceptionHandler:
    ...
    def test_handle_database_error_already_database(self):
        ...
        original_error = DatabaseError("已经是数据库错误")
    ...
    def test_handle_general_error(self, mock_logger):
        """测试处理通用异常"""
        original_error = ValueError("通用错误")
    ...
    def test_convenience_functions(self):
        ...
        with pytest.raises(DatabaseError) as cm:
            handle_create_error(ValueError("创建错误"), "客户", user_id=123)
        ...
        with pytest.raises(DatabaseError) as cm:
            handle_update_error(ValueError("更新错误"), "供应商", session_id="abc")
        ...
        with pytest.raises(DatabaseError) as cm:
            handle_query_error(ValueError("查询错误"), "报价")
    ...
    def test_log_error_warning_level(self, mock_logger):
        ...
        # WARNING级别不应该记录异常堆栈
        assert not call_args[1]["exc_info"]
